...
if (!function_exists('translate')){
    /**
     * Custom function for getting data from database.
     *
     * @param string
     * @return string
     */
...
if (! function_exists('makeData')) {
    /**
     * Custom function for verify data.
     *
     * @param  array  $mustValues
     * @param  array    $replaceTitles
     * @param  array    $post
     * @param  array    $date
     * @param  array    $add
     * @param  array    $ignore
     * @param  array    $makeInt
     * @param  array    $replaceCharset
     * @return array
     */
    function makeData($mustValues, $replaceTitles, $post, &$date, $add = array(), $ignore = array(),$makeInt = array(),$replaceCharset = array('`'=>'&#39;'))
    {
        $data = array();
        $values = array();
        foreach ($post as $k=>$v){
        ...
        foreach ($post as $k => $val) {
            if (in_array($val['name'],$makeInt)) $value = (int)$val['value']; else {
                $value = htmlspecialchars(trim($val['value']), ENT_QUOTES);
            }
            if (!in_array($val['name'],$ignore) and  in_array($val['name'], $mustValues) && (empty($value) && !in_array($val['name'],$makeInt))) {
                $date['error'] .= ', ' . $val['title'];
            }


            if (!in_array($val['name'],$ignore) and array_key_exists($val['name'], $replaceTitles)) $val['name'] = $replaceTitles[$val['name']];
...
if (!function_exists('logs')){
    ...
    function logs(){
        ...
        if(url()->current()!==url('/observer')) DB::table('logs')->insert(['userId'=>$user,'ip'=>$_SERVER['REMOTE_ADDR'],'lang'=>lang(),'page'=>url()->current()]);
    }
}